...
class MCPClient:
    ...
    async def process_query(self, query: str) -> str:
        ...
        } for tool in self.tools if tool.name not in ["show_eyes"]]
            ...
            if len(function_list)>0:
                ...
                for func in function_list:
                    ...
                    if function_name !='':
                        ...
                        print(f'⏳result: {function_response.content[0].text}')
        ...
        return result
    async def process_emotion(self, query: str) -> str:
        """使用 LLM 和 MCP 服务器提供的工具处理查询"""
        messages= self.messages[-10:] 
        ...
        emotion_prompt = os.environ["emotion_prompt"]
        # 初始化 LLM API 调用
        response_gen = await self.client.chat.completions.create(
            model="gpt-5-nano",
            messages=[{"role": "system", "content": emotion_prompt}]+ messages,
            tools=available_tools,
        ...
        function_response = response_gen.choices[0].message
        print(function_response)
        if function_response and function_response.tool_calls:
            for tool_call in function_response.tool_calls:
                tool_name = tool_call.function.name
                tool_args = json.loads(tool_call.function.arguments)
                if tool_name == "show_eyes":
                    if "emotion" in tool_args:
                        emotion = tool_args["emotion"]
                        print(f"show_eyes called with emotion: {emotion}")
                        await self.sessions[tool_name].call_tool(tool_name, {"emotion": emotion})
                        return f"show_eyes called with emotion: {emotion}"
    ...
    async def chat_loop(self):
        ...
        while True:  
            ...
            while unknownCount < 2:
                ...
                results=asyncio.gather(self.process_query(user_input), self.process_emotion(user_input))
                response=results[0]
